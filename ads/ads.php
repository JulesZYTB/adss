...
include("../include/config.php");
$query_keyinformation_views = "SELECT * FROM `views` where ads_id ='$ads_id'";
$result_keyinformation_views = mysqli_query($mysqli,$query_keyinformation_views);
$views_value = mysqli_fetch_array($result_keyinformation_views);
$views  = $views_value['views'];
$views_id  = $views_value['id'];
if($views_id>0){
$views++;
include("../include/config.php");
mysqli_query($mysqli,"SET NAMES 'utf8'");
$query_add_bank = "	update views set views='$views' where ads_id='$ads_id'";
...
include("../include/config.php");
mysqli_query($mysqli,"SET NAMES 'utf8'");
$query_add_bank = "	INSERT INTO `views`(`id`, `views`, `ads_id`) VALUES (NULL, \"$views\", \"$ads_id\");";
$execution_query_add_bank = mysqli_query($mysqli,$query_add_bank);
...
<link rel="shortcut icon" href="<?php echo $url_hraj; ?>templates/default/favicon.ico" type="image/x-icon">
<link rel="icon" href="<?php echo $url_hraj; ?>templates/default/favicon.ico" type="image/x-icon">
...
<!--Body content-->
	...
	if(isset($_GET['ban'])){
		$ban =$_GET['ban'];
		if($ban=='user' ){
			if(($group_num === "1" || $group_num === "2") and isset($_SESSION['id_members'])){
		
			$query_update_ads_ban = "UPDATE `members` set groupnumber = '5'  where id = \"$His_announcement_s\"";
			$execution_query_update_ads_ban = mysqli_query($mysqli,$query_update_ads_ban);
		
			$query_update_ads_ban = "UPDATE `ads` set close_ads = '6'  where His_announcement = \"$His_announcement_s\"";
			$execution_query_update_ads_ban = mysqli_query($mysqli,$query_update_ads_ban);
		
			echo '<div class="alert  alert-success">
			تم حظر العضو صاحب  الاعلان الحالي وايقاف جميع اعلاناتة بالموقع
<br>
			</div>
        ';
		}
...
<br>
		...
		<div class=" row ad_high">
			...
			<div class=" comment_header">
				 &nbsp; <a href="<?php echo $fixed_htaccess; ?>users/<?php echo $username_member_2_m_ad; ?>" class="username"><?php echo $username_member_2_m_ad; ?></a>      &nbsp; <?php echo $Time_added; ?></a> في <a href="<?php echo $fixed_htaccess; ?>city/<?php echo $ads_city; ?>/<?php echo $ads_city_name; ?>" class="city-head"><?php echo $ads_city_name; ?></a>
				<br>
				رقم الاعلان : <?php echo $ads_id; ?> | المشاهدات : <?=$views?>
                <?php 
	...
	if($if_found_ad === 1){
	?>
<a href="<?php echo $fixed_htaccess; ?>ads/<?php echo $new_id_link; ?>/<?php echo $new_title_link; ?>" class="nextad"> التالي     ←  </a>
<?php 
}
?>
  <br>
...
$cars = array_unique($IMAGES_LINK_ARRAY_P_2);
...
?>
   ...
   <span class="label label-success">وسيلة الإتصال :</span>   <strong> <a style="color: blue" href='tel:<?php echo $ads_contact; ?> '><?php echo $ads_contact; ?> </a>     <i class="fa fa-phone"></i></strong> 
   <br>  <br> 
   	<?php }else{ ?>
  ...
  <a href="<?php echo $fixed_htaccess; ?>close.php?ads_id=<?php echo $ads_id; ?>"><i class="fa fa-trash-o"></i> حذف الإعلان </a>
		...
		?>	
      		  <?php 
		if(($group_num === "1" || $group_num === "2") and isset($_SESSION['id_members'])){
			echo "<a href='?ban=user'><i class='fa fa-ban'></i> حظر العضو </a>  <br>";
		}
       ?>
...
<div class="input-icon-wrap">
  	  ...
  	  <a href="<?php echo $fixed_htaccess; ?>sendpm.php?username=<?php echo $username_member_2_m_ad; ?>&title=بخصوص إعلانك رقم <?php echo $ads_id; ?>"><i class="fa fa-envelope  fa-2x"></i> </a>


<a href="<?php echo $fixed_htaccess; ?>addfav.php?ads_id=<?php echo $ads_id; ?>" target="_blank"> <img src="/img/icon/addfav-icon.png" alt="" width="35" height="35" border="0" /></a>

  	<a class="input-with-icon2" href="<?php echo $fixed_htaccess; ?>report_ads.php?link=<?php echo $fixed_htaccess; ?>ads/<?php echo $ads_id; ?>/<?php echo $ads_title; ?>"><i class="fa fa-flag fa-2x"></i> </a>

<a href="https://www.facebook.com/sharer/sharer.php?u=" class="facebook-share-button" data-url="<?php echo $fixed_htaccess; ?>report_ads.php?link=<?php echo $fixed_htaccess; ?>ads/<?php echo $ads_id; ?>/<?php echo $ads_title; ?>" target="_blank"> <img src="/img/icon/facebook-icon.png" alt="" width="30" height="30" border="0" /></a>

<a class="input-with-icon2" href="https://twitter.com/share" class="twitter-share-button" data-url="<?php echo $fixed_htaccess; ?>report_ads.php?link=<?php echo $fixed_htaccess; ?>ads/<?php echo $ads_id; ?>/<?php echo $ads_title; ?>"><i class="fa fa-twitter  fa-2x"></i> </a>
			<!--  end -->
      ...
      <div align="center">
       <?php 
		...
		include("../include/config.php");
				...
				$view_query_bank_accounts = "SELECT * FROM `temp_Ad` 
				WHERE Ad_placement = \"ads_replay_tem\" ORDER BY Ad_order ASC";
				$unicode_arabic = mysqli_query($mysqli,"SET NAMES 'utf8'");
		...
		while($ad_header = mysqli_fetch_array($view_execution_bank_accounts)){
		?>
          
         <?php if($ad_header[Ad_time] > time()){  ?>          
         <?php if(!empty($ad_header[Image_Link])){ ?>
         
         <?php if(!empty($ad_header[Ad_link])){ ?>
         <a href="<?php echo $ad_header[Ad_link]; ?>" title="<?php echo $ad_header[Ad_title]; ?>">
         <?php } ?>
         
         <img src="<?php echo $ad_header[Image_Link]; ?>" class="img-responsive" style="padding:5px;"  alt="<?php echo $ad_header[Ad_title]; ?>" 
         <?php if($ad_header[Image_height] !== "0"){?> height="<?php echo $ad_header[Image_height]; ?>" <?php } ?>
         <?php if($ad_header[Image_width] !== "0"){?>  width="<?php echo $ad_header[Image_width]; ?>" <?php } ?> />
         ...
         <?php if(!empty($ad_header[Ad_html])){ ?>
		 <?php echo $ad_header[Ad_html]; ?>
         <br><br>
		 <?php } ?>
...
<?php if($group_num_m_ad02 === "1"){ ?><span style="background:#<?php echo $color1_group; ?>;" class="label label-success">الإدارة</span><?php } ?>
...
<?php if($group_num_m_ad02 === "2"){ ?><span class="label label-success">مشرف</span><?php } ?>
...
$fixed_htaccess/ads/$ads_id
            ...
            $url_website = $fixed_htaccess."ads/".$ads_id."/".$ads_title;
...
?>
           ...
           <br>
<div class="alert alert-info">
	يجب عليك <a href="<?php echo $fixed_htaccess; ?>login.php">تسجيل الدخول</a> حتى تتمكن من إضافة رد.
	</div>	
    <?php
			}
	?>
