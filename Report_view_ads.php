...
<title>قائمة الإبلاغات ( الأعلانات ) - <?php echo $namewebsite; ?></title> 
...
<div class="row">
	...
	<div class="col-xs-12  col-sm-8 col-md-9 col-lg-9 ">
		...
		<h3>قائمة الإبلاغات ( الأعلانات )</h3>
	...
	";
			...
			$query_pm_inbox_delete = "DELETE FROM `report_ads` WHERE id IN ($impid_pm)";
    ...
    <?php
				...
				mysqli_query($mysqli,"SET NAMES 'utf8'");
				$query_number_pm_inbox = "SELECT * FROM `report_ads` order by id DESC";
		    	...
		    	<th>رابط البلاغ</th>
		     	<th>السبب</th>
                <th>هل هذا الإعلان مخالف؟ </th>
                    <?php
				while($pm_inf = mysqli_fetch_array($result_query_number_pm_inbox)){
				
				
				$reson = $pm_inf[Reasons];
				$id_report = $pm_inf[id];
				$id_ads_del = $pm_inf[id_ads];
				$Contrary_to = $pm_inf[Contrary_to];
				if($Contrary_to === "yes"){$Contrary_to_arabic = "نعم";}
				if($Contrary_to === "no"){$Contrary_to_arabic = "لا";}
				...
				<tr><td><input type="checkbox" name="msg[]" value="<?php echo $id_report; ?>"></td><td>
                 <a href="<?php echo $id_ads_del; ?>">الأنتقال لرابط البلاغ</a>
                </td><td><?php echo $reson; ?></td>
               <td><?php echo $Contrary_to_arabic; ?></td>
					...
					?>
                    <div class="alert alert-info">لا يتوفر عناصر في قائمة الإبلاغات ( الأعلانات )</div>
					<?php
						}
					?>
