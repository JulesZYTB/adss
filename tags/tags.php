...
<a href="<?php echo $fixed_htaccess; ?>add.php" class="btn btn-success btn-lg "> <i class="fa fa-plus "></i> أضف إعلانك معنا</a>
		  ...
		  <button class="btn btn-primary active" onclick="location.href='<?php echo $url_hraj; ?>tags/<?php echo $tag_id; ?>/<?php echo $title_tag; ?><?php if(!empty($tag_model) and $tag_model !== "allmodels"){echo "/$tag_model";} ?><?php if(!empty($tag_city) and $tag_city !== "allcities"){echo "/$tag_city";} ?>'"><i class="fa fa-globe"></i></button>

		  <button class="btn btn-primary " onclick="location.href='<?php echo $url_hraj; ?>pic/<?php echo $tag_id; ?>/<?php echo $title_tag; ?><?php if(!empty($tag_model) and $tag_model !== "allmodels"){echo "/$tag_model";} ?><?php if(!empty($tag_city) and $tag_city !== "allcities"){echo "/$tag_city";} ?>'"> <i class="fa fa-camera-retro  "></i> </button>
...
<br>
	...
	$query_ads = "SELECT * FROM `ads` WHERE close_ads = \"0\" and $val_type_tags_search = \"$tag_id\" $tag_model_query 
	...
	mysqli_query($mysqli,"SET NAMES 'utf8'");
  	...
  	$query_ads = "SELECT * FROM `ads` WHERE close_ads = \"0\" and $val_type_tags_fixed = \"1\" and $val_type_tags_search = \"$tag_id\"

	$tag_model_query $tag_city_query ORDER BY Last_updated_Ad DESC";
 ...
 */
 ...
 $fixed_query_php = "and $val_type_tags_search = \"$tag_id\" and $val_type_tags_fixed = \"0\" $tag_model_query $tag_city_query";
...
<a href="<?php echo $url_hraj; ?>pic/<?php echo $tag_id; ?>/<?php echo $title_tag; ?><?php if(!empty($tag_model) and $tag_model !== "allmodels"){echo "/$tag_model";} ?><?php if(!empty($tag_city) and $tag_city !== "allcities"){echo "/$tag_city";} ?>"><i class="fa fa-camera-retro"></i> إعلانات مصورة عن  : 
