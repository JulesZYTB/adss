...
<title>قائمة حظر الرسائل الخاصة  - الأسباب - <?php echo $namewebsite; ?></title> 
...
<div class="row">
	...
	<div class="col-xs-12  col-sm-8 col-md-9 col-lg-9 ">
		...
		<h3>قائمة حظر الرسائل الخاصة</h3>
	...
	";
			...
			$query_pm_inbox_delete = "DELETE FROM `blocklistpm` WHERE id IN ($impid_pm)";
	...
	";
			...
			$impid_pm = secure_input($impid_pm,"text_input");
			$unicode_arabic = mysqli_query($mysqli,"SET NAMES 'utf8'");
			$query_pm_inbox_delete = "UPDATE `blocklistpm` 
    ...
    <?php
				...
				mysqli_query($mysqli,"SET NAMES 'utf8'");
				$query_number_pm_inbox = "SELECT * FROM `blocklistpm` where `read` = \"0\" order by id DESC";
				...
				<tbody><tr>
				<th>اختيار</th>
		    	<th>الذي قام بحظر العضو</th>
		     	<th>الشخص الذي تم حظره</th>
                <th>السبب</th>
                    <?php
			...
			mysqli_query($mysqli,"SET NAMES 'utf8'");
			$query_login_m_ad02_to = "SELECT * FROM `members` where id = \"$id_Taboo\" ";
			$result_query_m_ad02_to = mysqli_query($mysqli,$query_login_m_ad02_to);
			$Data_member_m_ad02_to = mysqli_fetch_array($result_query_m_ad02_to);
			$group_num_m_ad02_id_Taboo = $Data_member_m_ad02_to[groupnumber];
			$username_member_2_m_ad02_id_Taboo =  $Data_member_m_ad02_to[username];
			$id_user_m_ad02_id_Taboo =  $Data_member_m_ad02_to[id];	  
            ...
            <a href="users/<?php echo $username_member_2_m_ad02_id_Taboo; ?>" class="username"><div align="center"><?php echo $username_member_2_m_ad02_id_Taboo; ?></div></a>
                </td><td> 
               ...
               <td><?php echo $reson; ?></td>
					...
					?>
                    <div class="alert alert-info">لا يتوفر عناصر في قائمة حظر الرسائل الخاصة</div>
					<?php
						}
					?>
