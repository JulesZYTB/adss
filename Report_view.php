...
<title>قائمة الأبلاغات - <?php echo $namewebsite; ?></title> 
...
<div class="row">
	...
	للأسف لم تختار الإبلاغات التي تريد حذفها
	</div>
	";
		}else{
			...
			$query_pm_inbox_delete = "DELETE FROM `report` WHERE id IN ($impid_pm)";
			$execution_pm_inbox_delete = mysqli_query($mysqli,$query_pm_inbox_delete);
			if($execution_pm_inbox_delete){
				echo "
				<div class=\"alert alert-success\">تم حذف الإبلاغات المختارة</div>
				";
				}else{
						echo "
						<div class=\"alert alert-danger\">
						يبدو انه يوجد مشكلة في حذف الإبلاغات تواصل مع الأدارة
						</div>
						";
					}
			}
    ...
    <?php
				...
				mysqli_query($mysqli,"SET NAMES 'utf8'");
				$query_number_pm_inbox = "SELECT * FROM `report` order by id DESC";
				...
				<tbody><tr>
				<th>اختيار</th>
		    	<th>رقم الرد و الأعلان ( ID )</th>
		     	<th>صاحب الرد المبلغ عنه</th>
                <th>السبب</th>
                    <?php
				...
				$reson = $pm_inf[reson];
				if($reson === "1"){$reson_text = "بخس السلعة";}else{
				if($reson === "2"){$reson_text = "رد غير لائق";}else{
				if($reson === "3"){$reson_text = "عدم الجدية في الشراء";}else{$reson_text = "غير ذلك";}
				}
				}
				
				$comment_id = $pm_inf[id_comment];
				$id_report = $pm_inf[id];
			...
			$id_user_m_ad02 =  $Data_member_m_ad02[id];	 
                ...
                <a href="ads/<?php echo $id_ads; ?>?<?php echo $id_report; ?>#<?php echo $comment_id; ?>">الأنتقال للرد رقم ( <?php echo $comment_id; ?> )</a>
                |
                 <a href="ads/<?php echo $id_ads; ?>?<?php echo $id_report; ?>">الأنتقال للأعلان رقم ( <?php echo $id_ads; ?> )</a>
                </td><td> 
                <a href="users/<?php echo $username_member_2_m_ad02; ?>" class="username"><?php echo $username_member_2_m_ad02; ?></a></td>
               <td><?php echo $reson_text; ?></td>
                    ...
                    <tr>
		      		<td colspan="4">&nbsp;
		        	<input class="btn btn-danger" type="submit" name="remove" value="حذف الأبلاغات المختارة"></td></tr>
					</tbody></table>
					</div></form>
                    <?php
					}else{
					?>
                    <div class="alert alert-info">لا توجد إبلاغات</div>
					<?php
						}
					?>
