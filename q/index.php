...
if(!empty($key_act) and isset($key_act))
{
	...
	$phone = $fetch_array_active[phone];
		...
		if($groupnumber === "0")
		{
			if($_POST['submit'])
			{
				...
				if(empty($password_empty) and empty($username_empty) and empty($found_username))
				{
					$time_now_re = time(); 
					mysqli_query($mysqli,"SET NAMES 'utf8'");
					$query_insert_member_active = "UPDATE `members` set username = \"$username_new\" , password = \"$password_new_md5\" , groupnumber = \"6\" , timeregister = \"$time_now_re\" ,Documentingemail = \"1\" , sendtime = \"3\" , now = \"$password_new\" ,subscribe_1 = \"1\" , subscribe_2 = \"1\" , subscribe_3 = \"1\" where member_code = \"$key_act\" and groupnumber = \"0\" ";
					...
					{
						echo "<div class=\"alert alert-danger\">يبدو انه هناك مشكلة في التسجيل راسل الأدارة . </div>";
					}
				}
				else
				{
				...
				<div class="row">
					<div class="col-xs-12 col-sm-12 col-md-6 col-md-offset-3 col-lg-6 col-lg-offset-4">
						<form method="post" action="">
							...
							<div class="alert alert-info">يجب التسجيل بأسم لائق وذو معنى.</div>
							<br>
...
if($_POST['submit'])
{
	...
	$errors = array();
	$codes = array( 966, 973 );
	if( !in_array( $_POST['countrycode'], $codes)  )
	{
		$errors[] = 'Error in country code';
	}
	
	$countrycode = intval( $_POST['countrycode'] );
	if( $countrycode == 966 AND strlen( $phone < 10 ) )
	{
		$errors[] = 'Error in phone length';
	}
	if( $countrycode == 973 AND strlen( $phone < 8 ) )
	{
		$errors[] = 'Error in phone length';
	}
	...
	if( !empty( $errors ) )
	{
		echo "<div class=\"alert alert-danger\"> رقم الجوال خاطئ ! </div>";
	}
	else
	{
		...
		if($search_num > 0)
		{
			if($groupnumber === "0")
			{
				echo "
					...
					<br><b>رقم جوالك المسجل لدينا هو:</b> $inf_phone
					<br>
					</div>
					";
			}
		...
		else
		{
			mysqli_query($mysqli,"SET NAMES 'utf8'");
			$time_now = time();
			$Activation_code = md5($time_now.$phone);
			mysqli_query($mysqli,"SET NAMES 'utf8'");
			$query_new_member = "INSERT INTO `members` (groupnumber,phone,timeregister,member_code) VALUES (\"0\",\"$phone\",\"$time_now\",\"$Activation_code\")";	
			...
			if($result_new_member)
			{
				...
				$new_member = mysqli_query( $mysqli, $sql );
				$new_member = mysqli_fetch_array( $new_member );
				$Activation_code = $phone.$new_member['id'];

				// Update activation code
				$sql = "UPDATE members SET member_code = '{$Activation_code}' WHERE id = {$new_member['id']}";
				...
				$message = "رابط التسجيل الخاص في {$namewebsite}: $site_url/q/$Activation_code";
				$send = fastSendSMS( $message, $phone );

				if( $send == 'تمت عملية الإرسال بنجاح' )
				{
					echo "
						<div class=\"alert alert-success\">تم إرسال كود التفعيل ورابط التسجيل إلى جوالك، <br>يجب الدخول عليك الضغط على الرابط الذي تم إرساله 
				...
				else
				{
					...
					<br>نص الخطأ: $send <br><a href=\"contact.php\">إضغط هنا للأتصال بنا</a>
					</div>
					";
				}
			}
...
?>
	...
	<hr>
	<div class="alert alert-info">يجب ادخال رقم جوال صحيح حتى يتم إرسال رابط التسجيل إليك</div>
	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-6 col-md-offset-3 col-lg-6 col-lg-offset-4">
			<form method="post" action="">
				<div class="form-group">
					<label>رقم الجوال</label>
					<input type="text" name="phone" maxlength="50" placeholder="من فضلك ادخل رقم جوال صحيح مثال: 05999999999" class="form-control" value="">
				</div>
				<div class="form-group">
