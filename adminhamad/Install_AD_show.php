...
<title>عرض الأعلانات - التثبيت - لوحة التحكم</title>
...
<div class="full_w">
				<div class="h_title">عرض الأعلانات - التثبيت</div>
				<h2>عرض الأعلانات</h2>
				<p>
               يمكنك من هنا تثبيت الأعلانات 
                </p>
				...
				$view_query_bank_accounts = "SELECT * FROM `ads` ORDER BY ID";
	...
	if(empty($search_message)){}else{
		$search_message_query = "WHERE `ads_title` LIKE \"%$search_message%\" or `id` = \"$search_message\"";
    ...
    */
    $query = "SELECT COUNT(*) as num FROM `ads` $search_message_query";
    $total_pages = mysqli_fetch_array(mysqli_query($mysqli,$query));
    $total_pages = $total_pages[num];
    
    /* Setup vars for query. */
    $targetpage = "Install_AD_show.php?title="."$search_message"."";     //your file name  (the name of this file)
    ...
    /* Get data. */
    //$sql = "SELECT * FROM ads $search_message_query LIMIT $start, $limit";
    $sql = "SELECT * FROM ads $search_message_query ";
    ...
    */
	...
	?>
        ...
        ?>
							...
							<td><a href="Install_AD.php?id=<?php echo $row[id]; ?>">
							<?php echo $row[ads_title]; ?></a></td>
							<td><?php echo $row[type_ads_other_final]; ?></td>
