...
<title>حذف الأقسام عن - لوحة التحكم</title>
...
<div class="wrap">
	<div id="header">
		...
		<div id="main">
		  <div class="full_w">
		    <div class="h_title">حذف الأقسام عن</div>
            <?php
				$id_acc = $_GET['id'];
			...
			$query_add_bank = "SELECT * FROM `sectionoadvertise` WHERE id = \"$id_acc\"";
			...
			$unicode_arabic = mysqli_query($mysqli,"SET NAMES 'utf8'");
			$query_delet_bank = "DELETE FROM `sectionoadvertise` WHERE `id` = \"$id_acc\" LIMIT 1;";
			...
			if($execution_query_delet_bank){
						...
						echo "
						<div class=\"n_error\"><p>
						للأسف لا يمكن الحذف حاول في وقت لاحق
						</p></div>
						";
					}
								...
								echo "
						<div class=\"n_error\"><p>
						للأسف لا يمكن الحذف لأن المطلوب حذفه غير موجود
						</p></div>
						";
