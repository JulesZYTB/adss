...
<title>عرض السنوات - لوحة التحكم</title>
...
<div class="full_w">
				<div class="h_title">عرض السنوات - التعديل - الحذف </div>
				<h2>عرض السنوات</h2>
				<p>
                يتم هنا تعديل السنوات و حذفها
                </p>
				...
				$view_query_bank_accounts = "SELECT * FROM `years` ORDER BY text";
    ...
    */
    ...
    /* Get data. */
    $sql = "SELECT * FROM years order by text DESC $search_message_query LIMIT $start, $limit";
    ...
    */
	...
	?>
        ...
        ?>
							...
							<td><a href="edit_years.php?id=<?php echo $row[id] ?>">
							<?php echo $row[text] ?></a></td>
							<td>
					<a onClick="return confirm('هل أنت متأكد من رغبتك في حذف السنة ؟')" 
                    href="Delete_years.php?id=<?php echo $row[id] ?>" class="table-icon delete" title="حذف"></a>
							</td>
						</tr>
