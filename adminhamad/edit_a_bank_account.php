...
<title>تعديل بنك - لوحة التحكم</title>
...
<div class="wrap">
	<div id="header">
		...
		<div id="main">
		  <div class="full_w">
		    <div class="h_title">تعديل حساب بنكي</div>
            <?php
				$id_acc = $_GET['id'];
			...
			$query_add_bank = "UPDATE `bankaccounts` set 
			his_bank_account = \"$his_bank_account\" 
			, Bank_Name = \"$Bank_Name\"
			, Account_Number = \"$Account_Number\"
			, Account_Number_International = \"$Account_Number_International\" 
			where id = $id_acc";
			...
			if($execution_query_add_bank){
						echo "
						<div class=\"n_ok\"><p>
						تم تعديل البنك بنجاح جاري تحويلك لصفحة عرض البنوك . 
						</p></div>
						";
						...
						echo "
						<div class=\"n_error\"><p>
						للأسف لم يتم تعديل البنك يبدو انه هناك مشكلة حاول في وقت لاحق . 
						</p></div>
						";
					}
				...
				mysqli_close($mysqli);
								...
								/// استعدعاء ملف الأتصال بقاعدة البيانات
				include("../include/config.php");
				$view_query_bank_accounts = "SELECT * FROM `bankaccounts` WHERE id = \"$id_acc\"";
				$unicode_arabic = mysqli_query($mysqli,"SET NAMES 'utf8'");
				$view_execution_bank_accounts = mysqli_query($mysqli,$view_query_bank_accounts);
				$bank_acc = mysqli_fetch_array($view_execution_bank_accounts);	
			...
			?>
				<form action="" method="post">
					...
					<div class="element">
						...
						<input id="name" name="his_bank_account" value="<?php echo $bank_acc[his_bank_account]; ?>" maxlength="150" class="text err" />
					</div>
                    
                    					<div class="element">
						<label for="name">اسم البنك <span class="red">(مطلوب)</span></label>
						<input id="name" name="Bank_Name" value="<?php echo $bank_acc[Bank_Name]; ?>" maxlength="150" class="text err" />
					</div>
						...
						<input id="name" name="Account_Number" value="<?php echo $bank_acc[Account_Number]; ?>" maxlength="150" class="text" />
					</div>
						...
						<input id="name" name="Account_Number_International" value="<?php echo $bank_acc[Account_Number_International]; ?>" maxlength="150" class="text" />
					</div>
					...
					<div class="entry">
					 <input type="submit" class="add_Account" name="ADD_Account" value="تعديل البنك" />
					</div>
