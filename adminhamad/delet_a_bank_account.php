	...
	<div id="header">
		...
		<div id="main">
		  <div class="full_w">
			...
			$query_add_bank = "SELECT * FROM `bankaccounts` WHERE id = \"$id_acc\"";
			...
			$unicode_arabic = mysqli_query($mysqli,"SET NAMES 'utf8'");
			$query_delet_bank = "DELETE FROM `bankaccounts` WHERE `id` = \"$id_acc\" LIMIT 1;";
			...
			if($execution_query_delet_bank){
						echo "
						<div class=\"n_ok\"><p>
						تم حذف البنك بنجاح جاري تحويلك على صفحة عرض البنوك 
						</p></div>
						";
						...
						echo "
						<div class=\"n_error\"><p>
						للأٍسف لم يتم حذف البنك يبدو انه يوجد مشكلة حاول في وقت لاحق
						</p></div>
						";
					}
								...
								echo "
						<div class=\"n_error\"><p>
						للأسف البنك الذي تريد حذفه غير موجود - جاري تحويلك لصفحة عرض البنوك
						</p></div>
						";
